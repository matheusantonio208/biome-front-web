...
const EcommerceOrders = () => {
  ...
  const handleOrderClick = useCallback((arg) => {
    ...
    const today = new Date(order.orderDate);
    const yyyy = today.getFullYear();
    let mm = today.getMonth() + 1; // Months start at 0!
    let dd = today.getDate();
    const updateOrderDate = yyyy + '-' + mm + '-' + dd;
    console.log(updateOrderDate);
    ...
    // console.log("order", moment(new Date(order.orderDate)).format("dd/mm/yyyy"));
